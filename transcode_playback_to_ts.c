...
#if BTST_ENABLE_USER_DATA_ENCODE
    {NEXUS_VideoFormat_e720p, NEXUS_VideoFrameRate_e60, 10*1000*1000, 60, 0, NEXUS_VideoCodec_eMpeg2, NEXUS_VideoCodecProfile_eMain, NEXUS_VideoCodecLevel_eHigh},
#else
...
again:
   ...
   if(platformConfig.outputs.component[0]){
...
#if SEGMENTED_RECORD
	{
		char ts_name[255];
		char index_name[255];
		sprintf(ts_name, "/data/temp/%s/%s.ts", streamSettings.fname, streamSettings.fname);
		sprintf(index_name, "/data/temp/%s/%s.nav", streamSettings.fname, streamSettings.fname);
